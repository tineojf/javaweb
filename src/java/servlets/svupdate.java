...
public class svupdate extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        AsociadoModel associate = dao.findByID(id);

        if (associate != null) {
            request.setAttribute("associate", associate);
            RequestDispatcher dispatcher = request.getRequestDispatcher("routes/update.jsp");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        AsociadoModel asociado = new AsociadoModel(id, name, lastname, status, eps,
                dni, contribution, level, salary, childen, phone, address);

        AsociadoDAO dao = new AsociadoDAO();

        String httpcode = dao.update(id, asociado);

        if (httpcode.equals("202: UPDATED")) {
            String mensaje = "Asociado actualizado";
            request.setAttribute("mensaje", mensaje);
